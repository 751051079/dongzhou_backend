...
@Component
public class SyncMercadoMarketPlaceItems {

    private static final Logger logger = LoggerFactory.getLogger(SyncMercadoMarketPlaceItems.class);
    ...
    public void syncMercadoMarketPlaceItems() {
        logger.info("同步站点产品定时任务启动=============");
        Lock lock = redisLockRegistry.obtain("syncMercadoMarketPlaceItems");
        boolean b = lock.tryLock();
        try {
            if (b) {
                MercadoCallBack example = new MercadoCallBack();
                example.setTopic("marketplace_items");
                String limit = redisService.getCacheObject("sys_config:process.market.place.items");
                ...
                if (!mercadoCallBacks.isEmpty()) {
                    ...
                    for (int index = 0; index < mercadoCallBacks.size(); index++) {
                        ...
                        amqpTemplate.convertAndSend(RabbitMQConstant.MQ_PULL_MERCADO_PRODUCT_EXCHANGE,
                                RabbitMQConstant.RoutingKey.MERCADO_CALL_BACK_INFO_MARKETPLACE_ITEMS, message);
