...
@Component
public class MercadoBatchCollectionLinkToShopListener {

    private static final Logger logger = LoggerFactory.getLogger(MercadoBatchCollectionLinkToShopListener.class);
    ...
    @RabbitListener(bindings = @QueueBinding(exchange = @Exchange(
            ...
            value = @Queue(name = RabbitMQConstant.MercadoQueue.CLOUD_PULL_MERCADO_BATCH_COLLECTION_LINK_TO_SHOP_QUEUE),
            key = RabbitMQConstant.RoutingKey.CLOUD_PULL_MERCADO_BATCH_COLLECTION_LINK_TO_SHOP))
    public void batchCollectionLink(BatchCollectionLinkToShop batchCollectionLinkToShop) {
        try {
            ...
            MercadoBatchCollectionLink mercadoBatchCollectionLink = mercadoBatchCollectionLinkMapper.selectMercadoBatchCollectionLinkById(batchCollectionLinkToShop.getLinkId());
            logger.info("查询到的链接信息 {}", JSON.toJSONString(mercadoBatchCollectionLink));
            if (mercadoBatchCollectionLink != null) {
                // 根据deptId获取token
                Long deptId = mercadoBatchCollectionLink.getDeptId();
                logger.info("获取到的部门id {}", deptId);
                MercadoShopInfo tokenInfo = mercadoProductMapper.getMercadoToken(deptId);
                if (tokenInfo != null) {
                    ...
                    if (token != null) {
                        // 根据MLM获取全局产品id
                        JSONObject parentJson = MercadoHttpUtil.getGlobalAndSiteRelationships(mercadoBatchCollectionLink.getItemId(), token);
                        ...
                        if (parentJson != null) {
                            ...
                            if (cbtId != null && itemId != null) {
                                // 获取全局产品信息
                                JSONObject productInfo = MercadoNewHttpUtil.getProductInfo(cbtId, token);
                                // 获取站点产品信息
                                JSONObject itemInfo = MercadoNewHttpUtil.getProductInfo(itemId, token);
                                if (productInfo != null && itemInfo != null) {
                                    ...
                                    mercadoProduct.setUserId(mercadoBatchCollectionLink.getUserId());
                                    mercadoProduct.setDeptId(mercadoBatchCollectionLink.getDeptId());
                                    ...
                                    mercadoProduct.setCreateBy(String.valueOf(mercadoBatchCollectionLink.getUserId()));
                                    mercadoProduct.setUpdateTime(new Date());
                                    mercadoProduct.setUpdateBy(String.valueOf(mercadoBatchCollectionLink.getUserId()));
                                    ...
                                    if (itemInfo != null) {
                                        ...
                                        JSONArray attributesProductInfo = productInfo.getJSONArray("attributes");
                                    ...
                                    if (result > 0) {
                                        ...
                                        String priceRatio = redisService.getJSONStringByKey("DEPT_PRICE_RATIO" + mercadoBatchCollectionLink.getDeptId());
                                        if (priceRatio != null) {
                                            BigDecimal sitePrice = itemInfo.getBigDecimal("price").divide(new BigDecimal(usdExchangeRate), 2, BigDecimal.ROUND_HALF_DOWN).multiply(new BigDecimal(priceRatio)).setScale(2, RoundingMode.HALF_UP);
                                            mercadoProductItem.setSiteSalePrice(sitePrice);
                                        } else {
                                            BigDecimal sitePrice = itemInfo.getBigDecimal("price").divide(new BigDecimal(usdExchangeRate), 2, BigDecimal.ROUND_HALF_DOWN).multiply(new BigDecimal(increaseFactor)).setScale(2, RoundingMode.HALF_UP);
                                            mercadoProductItem.setSiteSalePrice(sitePrice);
                                        }
                                        // 获取站点描述
                                        JSONObject des = MercadoHttpUtil.getDesByCbtId(token, MERCADO_URL, cbtId, itemInfo.getString("site_id"), itemInfo.getJSONObject("shipping").getString("logistic_type"));
                                        if (des != null) {
