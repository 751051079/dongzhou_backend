<template>
	<div class="app-container">
		...
		<el-row :gutter="10" class="mb8">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-refresh" size="mini" @click="handleAdd">更新店铺链接
				</el-button>
				<el-button type="primary" icon="el-icon-plus" size="mini" @click="onCollect"
					:disabled="tableSelection.length === 0">采集链接
				</el-button>
		...
		<el-table v-loading="loading" :data="shopList" :default-sort="defaultSort" @cell-click="onCellClick"
			...
			<el-table-column label="产品信息" align="left" :show-overflow-tooltip="true" width="450px">
				<template slot-scope="scope">
					<div class="click-link" @click="onClickToPage(scope.row)">{{ scope.row.id }}</div>
					<div>售价:{{ scope.row.price }} USD</div>
			...
			<el-table-column label="卖家信息" align="center" :show-overflow-tooltip="true">
				<template slot-scope="scope">
					<div>卖家ID&nbsp;&nbsp;&nbsp;{{ scope.row.sellerId }}</div>
					<div>卖家名称&nbsp;&nbsp;&nbsp;{{ scope.row.sellerName }}</div>
				</template>
			</el-table-column>
			<el-table-column label="站点信息" align="center" :show-overflow-tooltip="true">
				<template slot-scope="scope">
					...
					<div>{{ scope.row.logisticType === 'remote' ? '自发货' : '海外仓' }}</div>
				</template>
			</el-table-column>
			<el-table-column label="状态" align="center">
				<template slot-scope="scope">
					<span>{{ scope.row.status === 'active' ? '激活' : '暂停' }}</span>
				</template>
			</el-table-column>
			<el-table-column label="销量" align="center" prop="soldQuantity" sortable="custom"
				:sort-orders="['descending', 'ascending']"></el-table-column>
			<el-table-column label="创建时间" align="center" prop="dateCreated" sortable="custom"
				:sort-orders="['descending', 'ascending']">
				<template slot-scope="scope">
					<span>{{ scope.row.dateCreated }}</span>
				</template>
			</el-table-column>
			<el-table-column label="更新时间" align="center" prop="lastUpdated" sortable="custom"
				:sort-orders="['descending', 'ascending']">
				<template slot-scope="scope">
...
<script>
import { listItem, selectListShop, deleteItem, updateLink, batchUpdate, collectLink } from "@/api/mercado/item"
			...
			this.$refs['elForm'].validate((valid) => {
				if (valid) {
					...
					if (this.shopType === 'update') {
						updateLink(merchantId).then((res) => {
							if (res.code == 200) {
